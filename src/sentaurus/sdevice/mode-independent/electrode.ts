type ElectrodeType = { Name: string } & (
    ElectrodeVoltage | ElectrodeCurrent | ElectrodeCharge
) &
    ElectrodeMisc;
...
type SchottkyType =
    { Schottky?: never } | { Schottky: true; barrier: BarrierType };
...
const formatElectrode = (e: ElectrodeType) => {
    ...
    switch (e.kind) {
                ...
                switch (b.kind) {
                    case "material":
                        if (typeof b.Material === "string")
                            retval.push(`Material="${b.Material}"`);
                        else
                            retval.push(
                                `Material="${b.Material.material}"(${b.Material.doping_type}=${b.Material.doping_conc})`,
                            );
                        break;
                    case "barrier":
        ...
        case "charge":
            ...
            if (e.FGcap)
                retval.push(
                    `FGcap=(value=${e.FGcap.value} name="${e.FGcap.name}")`,
                );
            if (e.CombineSemiAndContactCharges)
                retval.push("CombineSemiAndContactCharges");
            break;
    }
...
const formatPWL = (pwl: number | { [key: number]: number }) => {
    ...
    const pairs = Object.entries(pwl).map(([k, v]) => `${v} at ${k}`);
    return `( ${pairs.join(", ")} )`;
};

const electrodeGenerator = (ctx: string[]) => {
    const electrode = (...args: ElectrodeType[]) => {
        ctx.push("Electrode", "{", ...args.map(formatElectrode).flat(), "}");
    };
    return electrode;
};
