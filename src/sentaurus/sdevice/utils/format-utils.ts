...
const useFormatUtils = (ctx: string[]) => {
    const formatFlag = <
        ...
        G extends KeysWithValue<T, true | undefined>,
    >(
        raw: T,
        field: G,
    ) => {
    ...
    const formatSwitch = <
        T extends Record<string, unknown>,
        G extends KeysWithValue<T, boolean | undefined>,
    >(
        raw: T,
        field: G,
    ) => {
    ...
    const formatAssignment = <
        T extends Record<string, unknown>,
        G extends KeysWithValue<T, number | string | undefined>,
    >(
        raw: T,
        field: G,
        formatter?: (k: G, v: NonNullable<T[G]>) => string,
    ) => {
        const v = raw[field];
    ...
    const formatBlock = <
        T extends Record<string, unknown>,
        G extends KeysWithValue<T, unknown>,
    >(
        raw: T,
        field: G,
        formatter: (raw: NonNullable<T[G]>) => string[],
    ) => {
        const v = raw[field];
        if (v !== undefined && v !== null)
            ctx.push(field as string, "(", ...formatter(v), ")");
    };
